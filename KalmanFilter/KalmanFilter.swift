...
public struct KalmanFilter<Type: KalmanInput>: KalmanFilterType {
     ...
     */
    public func predict(stateTransitionModel: Type, controlInputModel: Type, controlVector: Type, covarianceOfProcessNoise: Type) throws -> KalmanFilter {
        // x̂_k|k-1 = F_k * x̂_k-1|k-1 + B_k * u_k
        let predictedStateEstimate = try stateTransitionModel * stateEstimatePrior + controlInputModel * controlVector
        // P_k|k-1 = F_k * P_k-1|k-1 * F_k^t + Q_k
        let predictedEstimateCovariance = try stateTransitionModel * errorCovariancePrior * stateTransitionModel.transposed() + covarianceOfProcessNoise
     ...
     */
    public func update(measurement: Type, observationModel: Type, covarienceOfObservationNoise: Type) throws -> KalmanFilter {
        // H_k^t transposed. We cache it improve performance
        let observationModelTransposed = try observationModel.transposed()
        
        // ỹ_k = z_k - H_k * x̂_k|k-1
        let measurementResidual = try measurement - observationModel * stateEstimatePrior
        // S_k = H_k * P_k|k-1 * H_k^t + R_k
        let residualCovariance = try observationModel * errorCovariancePrior * observationModelTransposed + covarienceOfObservationNoise
        // K_k = P_k|k-1 * H_k^t * S_k^-1
        let kalmanGain = try errorCovariancePrior * observationModelTransposed * residualCovariance.inversed()
        
        // x̂_k|k = x̂_k|k-1 + K_k * ỹ_k
        let posterioriStateEstimate = try stateEstimatePrior + kalmanGain * measurementResidual
        // P_k|k = (I - K_k * H_k) * P_k|k-1
        let posterioriEstimateCovariance = try (kalmanGain * observationModel).additionToUnit() * errorCovariancePrior
